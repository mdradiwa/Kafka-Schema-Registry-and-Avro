...
def send_record():
    ...
    file = open('data/bitcoin_price_Training_Cleaned.csv')
    csvreader = csv.reader(file)
    header = next(csvreader)
    for row in csvreader:
        key = {"Date":  str(row[0])}
        value = {"Date": str(row[0]), "Open": float(row[1]), "High": float(row[2]), "Low": float(row[3]), "Close": float(row[4]), "Volume": int(row[5]), "MarketCap": int(row[6])}
